...
for b, batchSize in enumerate(batch_sizes):
    ...
    # invert training images
    for i, data in enumerate(trainloader):
        inputs, labels = data
        pilInputs = []
        for img in inputs:
            pilInputs.append(transforms.ToPILImage(mode=None).__call__(img))
        invPilInputs = []
        for pil in pilInputs:
            con = PIL.ImageOps.autocontrast(pil)         
            invPilInputs.append(PIL.ImageOps.invert(con))
        invInputs = []
        for inv in invPilInputs:
            invInputs.append(transforms.ToTensor().__call__(inv))
            # imshow(inv)
        imshow(torchvision.utils.make_grid(invInputs))       
        data = invInputs, labels


    print("SUCCESS! Training Images Inverted.")    
    ...
    for i, data in enumerate(testloader):
        ...
        invPilInputs = []
        for pil in pilInputs:
            invPilInputs.append(PIL.ImageOps.invert(pil))
        invInputs = []
        for inv in invPilInputs:
            invInputs.append(transforms.ToTensor().__call__(inv))
        data = invInputs, labels
    
    print("SUCCESS! Test Images Inverted.")
