...
for b, batchSize in enumerate(batch_sizes):
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        plt.title("Loss by Epoch (Batch Size = %d)" % (batchSize))
        # plt.show()
        plt.savefig("loss_by_epoch_batchSize_%d.png" % (batchSize))
    ...
    # for i, label in enumerate(epoch_accuracy):
    #     plt.annotate(label, epoch_accuracy[i])
    plt.xlim(left=1)
    plt.ylim(bottom = min(epoch_accuracy), top = max(epoch_accuracy))
    plt.title("Epoch Accuracy (Batch Size = %d)" % (batchSize))
    plt.savefig("epoch_accuracy_batchSize_%d.png" % (batchSize))
